...
function onGpsPosition(pos){
  ...
  setGpsBadge(
    speedKmh === null ? `GPS live · ±${Math.round(currentPosition.accuracy)}m`
...
function renderSettings(){
  ...
  const q=["voice_alerts","background_driving_mode","community_reports","camera_layer","traffic_layer","hazard_layer"];

  byId("quickSettings").innerHTML=q.map(k=>
    typeof s[k]==="boolean" ? boolRow(k,s[k]) : scalarRow(k,s[k])
  ).join("");

  byId("allSettings").innerHTML=Object.entries(s).map(([k,v])=>
    typeof v==="boolean" ? boolRow(k,v) : scalarRow(k,v)
  ).join("");
}
...
function renderCameras(){
  const rows=adminData.cameras.map(c=>`<tr>
    <td>${esc(c.camera_type)}</td>
    <td>${esc(c.location)}</td>
    <td>${esc(c.speed_limit??"—")}</td>
...
async function addCamera(){
  ...
  if(r.ok){
    ["cameraLocation","cameraSpeed","cameraLat","cameraLng"].forEach(
      id=>byId(id).value=""
    );
    await loadAdmin();
  }
...
function renderUsers(){
  ...
  byId("appUsersTable").innerHTML=`<table>
    ...
    <tbody>
      ${appRows || '<tr><td colspan="3">No registered app users yet.</td></tr>'}
    </tbody>
  </table>`;
}
